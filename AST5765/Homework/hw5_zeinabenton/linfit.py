...
def linfit(y, x=None, y_unc=None):
  '''
    ...
    y: ndarray
        Ordinates, any shape.
    x: ndarray
        Abcissas, same shape.  Defaults to np.arange(len(y),
        dtype=y.dtype), reshaped.
    y_unc: ndarray
        Uncertainties in y.  If scalar or 1-element array, applied
        uniformly to all y values.  Must be positive.
...
(1.272727272727272, 1.9393939393939394, 0.58775381364525869, 0.11009637651263605, 9.6969696969696937, 0.28694204178663996, array([[ 0.34545455, -0.05454545],
    ...
    2010-12-15 0.5  jh@physics.ucf.edu  Fixed y.length bug.
    2011-10-04 0.6  jh@physics.ucf.edu  Fixed y_unc scalars bug.
    2017-09-26 0.7  jh@physics.ucf.edu  Python 3 conversion (None comparisons)
  '''
  # standardize and test inputs
  if x is None:
    x = np.arange(len(y), dtype=y.dtype)
    x.shape = y.shape
  
  if y_unc is None:
      y_unc = np.ones(y.shape, dtype=y.dtype)
  
  y_unc_i = y_unc * np.ones(y.shape, dtype=y.dtype) # ensure array for y_unc
